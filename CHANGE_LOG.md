...
- 新增：截图编辑模式。截图后保留绿框，允许微调选区位置和大小后再确认。
- 新增：八角拖拽手柄。选区四角和四边中点均可拖拽调整大小。
- 新增：选区移动功能。拖拽选区内部可移动整个选区位置。
- 新增：双击确认截图。在编辑模式下双击选区内部即可确认。
- 新增：键盘快捷操作。编辑模式下按 Enter 确认，Esc 取消。
- 优化：截图交互体验，避免误操作导致截图失败。
